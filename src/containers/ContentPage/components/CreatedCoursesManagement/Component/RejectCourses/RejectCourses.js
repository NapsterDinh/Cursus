...
// style component
import Wrapper from "./RejectCoursesStyled";

function RejectCourses() {
  ...
  const getDataRejectedCourses = async () => {
    setIsLoading(true);
    const response = await getAllProcessedCourses(2);
    setRejectedCourses(response?.data?.data?.result);
    setIsLoading(false);
  };

  useEffect(() => {
    getDataRejectedCourses();
  }, []);
  ...
  const data = [...rejectedCourses].reverse()?.map((item, index) => {
    return {
      key: item?.id,
      itemNo: index + 1,
